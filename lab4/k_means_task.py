...
class KMeans:
    ...
    def __init__(self, n_clusters: int):
        ...
        """
        self._max_iters: int = 1000

    @property
    def max_iters(self) -> int:
        """
        Просто геттер для "_max_iters".
        """
        return self._max_iters

    @max_iters.setter
    def max_iters(self, value: int) -> None:
        self._max_iters = int(value)
    ...
    def n_samples(self) -> int:
        """
        Количество записей в массиве данных.
        Например, количество {x, y} координат на плоскости.
        """
    ...
    def clusters(self) -> List[np.ndarray]:
        ...
        """
        return [np.array(self._data[ids, :]) for ids in self._clusters_points_indices]
    ...
    def _get_closest_cluster_center(self, sample: np.ndarray) -> int:
        ...
        """
        m = min([(distance(cluster_center, sample), cluster_center_index)
                 for cluster_center_index, cluster_center in enumerate(self._clusters_centers)], key=lambda x: x[0])
        return m[1]
